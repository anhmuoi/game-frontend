...
export function* putMarketUpdate(action) {
  ...
  let groupUser = action.groupUser;
  try {
    const res = yield call(request, urlUpdateMarket, option('PUT', groupUser));
    // model invalid
...
export function* postMarketAdd(action) {
  ...
  try {
    const res = yield call(request, urlAddMarket, option('POST', groupUser));
 ...
 */
export default function* groupUserData() {
  yield takeLatest(FETCH_GROUP_USER_INIT, getMarketInit);
  yield takeLatest(FETCH_GROUP_USER_DATA, getMarketData);
  yield takeLatest(UPDATE_GROUP_USER, putMarketUpdate);
  yield takeLatest(ADD_GROUP_USER, postMarketAdd);
  yield takeLatest(REQUEST_JOIN_GROUP, requestJoinGroup);
  yield takeLatest(INIT_INDEX_GROUP_USER, getInitIndexMarket);
  yield takeLatest(DELETE_GROUP_USER_ROW, deleteMarket);
  yield takeLatest(DELETE_MULTIES_GROUP_USER, deleteMultiesMarket);
  yield takeLatest(DELETE_MULTIES_GROUP_USER_SUCCESS, getMarketData);
  yield takeLatest(DELETE_GROUP_USER_SUCCESS, getMarketData);
  yield takeLatest(UPDATE_GROUP_USER_SUCCESS, getMarketData);
  yield takeLatest(CREATE_GROUP_USER_SUCCESS, getMarketData);
}

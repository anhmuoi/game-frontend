...
export class MarketInformationPage extends React.Component {
  ...
  deleteMultiesMarket = () => {
    const { groupUserIdSelected } = this.state;
    this.props.onDeleteMultiesMarket(groupUserIdSelected);
  };

  rowsSelected = (ids) => this.setState({ groupUserIdSelected: ids });
  ...
  getSortColumn = (sortColumn, value) => {
    const cloneSortColumn = API_COLUMNS.GROUP_USERS.indexOf(value);
  ...
  sellItemNft = () => {
    ...
    const nftApi = mapModelMarketUiToApi(
      this.props.groupUserDataModified.toJS(),
    );
  ...
  render() {
    ...
    return (
      <div className="room-game">
        ...
        >
          <FormattedMessage {...messages.groupUser} />
        </Typography>
        ...
        <div className="room-game-list" style={{ marginTop: 80 }}>
          {datas.map((group, key) => (
            ...
            >
              ...
              {user && group.id === user.departmentId ? (
                <div
                  style={{
                    padding: 10,
                    background: '#115651',
                ...
                <div
                  onClick={() => {
                    this.setState({
                      listSendRequest: [
                        ...this.state.listSendRequest,
                        group.id,
                      ],
                    });
                    this.props.onRequestJoinGroup(
                      group.id,
                ...
                >
                  {this.state.listSendRequest.includes(group.id) ? (
...
function mapDispatchToProps(dispatch) {
  return {
    ...
    // modified Market
    onUpdateMarket: (id, groupUser) => dispatch(putMarketUpdate(id, groupUser)),
    onAddMarket: (groupUser) => dispatch(postMarketAdd(groupUser)),
...
const mapStateToProps = createStructuredSelector({
  ...
  groupUserDataModified: getMarketDataModified(),
  userList: getUserListData(),
});

const withReducer = injectReducer({ key: 'groupUser', reducer });

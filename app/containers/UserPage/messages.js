import { defineMessages } from 'react-intl';

export const scope = 'demasterpro.containers.userPage';
export default defineMessages({
  btnExport: {
    id: `demasterpro.components.Common.export`,
    defaultMessage: `Export`,
  },
  btnImport: {
    id: `demasterpro.components.Common.import`,
    defaultMessage: `Import`,
  },
  titleFormatFile: {
    id: `demasterpro.components.Common.titleFormatFile`,
    defaultMessage: `Format file`,
  },

  titleToolBarDelete: {
    id: `demasterpro.components.Common.delete`,
    defaultMessage: `Delete`,
  },
  confirmDelete: {
    id: `demasterpro.components.Common.confirmDelete`,
    defaultMessage: `Do you want to delete?`,
  },

  btnCancel: {
    id: `demasterpro.components.Common.cancel`,
    defaultMessage: 'Cancel',
  },
  btnSave: {
    id: `demasterpro.components.Common.save`,
    defaultMessage: 'Save',
  },
  personalInfo: {
    id: `${scope}.personalInfo`,
    defaultMessage: `Personal Information`,
  },

  titleUser: {
    id: `${scope}.titleUser`,
    defaultMessage: `User`,
  },
  name: {
    id: `${scope}.name`,
    defaultMessage: 'Name',
  },

  position: {
    id: `${scope}.position`,
    defaultMessage: 'Position',
  },
  email: {
    id: `${scope}.email`,
    defaultMessage: 'Email',
  },

  status: {
    id: `${scope}.status`,
    defaultMessage: 'Status',
  },
  departmentId: {
    id: `${scope}.departmentId`,
    defaultMessage: `Department`,
  },
  phone: {
    id: `${scope}.phone`,
    defaultMessage: `Phone`,
  },
  holiday: {
    id: `${scope}.holiday`,
    defaultMessage: `Holiday`,
  },

  confirmEditUser: {
    id: `${scope}.confirmEditUser`,
    defaultMessage: `Do you want to edit User information?`,
  },
  confirmAddUser: {
    id: `${scope}.confirmAddUser`,
    defaultMessage: `Do you want to add User information?`,
  },

  placeholderSearchBox: {
    id: `${scope}.placeholderSearchBox`,
    defaultMessage: `Name`,
  },
  userName: {
    id: `${scope}.userName`,
    defaultMessage: `Username`,
  },
  password: {
    id: `${scope}.password`,
    defaultMessage: `Password`,
  },
  confirmPassword: {
    id: `${scope}.confirmPassword`,
    defaultMessage: `Confirm Password`,
  },
  authInfo: {
    id: `${scope}.authInfo`,
    defaultMessage: `Auth Information`,
  },
  role: {
    id: `${scope}.role`,
    defaultMessage: `Role`,
  },
  passwordNotMatch: {
    id: `${scope}.passwordNotMatch`,
    defaultMessage: 'Confirm password is not matched with Password.',
  },
  invalidPassword: {
    id: `${scope}.invalidPassword`,
    defaultMessage: 'Password can not be blank.',
  },
  invalidConfirmPassword: {
    id: `${scope}.invalidConfirmPassword`,
    defaultMessage: 'Confirm Password can not be blank.',
  },
});

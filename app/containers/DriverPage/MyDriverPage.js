...
import { generateIconDocument, generateIconDocumentFolder, generateSizeFile, changeSelectedIds, convertDateTimeDisplay } from './DriverUtil.js';
...
export class MyDriverPage extends React.Component {
  state = {
  ...
  componentDidMount = async () => {
    if (window.location.pathname.includes('folders')) {
      ...
      if (match && match.params && match.params.id) {
        await this.props.onGetInit(match.params.id);
      }
    } else {
      await this.props.onGetInit();
    }
  }
  ...
  openFolderRow = (item) => {
    if (item.folderId) {
      ...
      this.props.onAddItemToListLinkFolder([...listLinkFolders, { id: item.folderId, name: item.name }]);
    }
  }
        ...
        <Grid container spacing={24}>
          ...
          <Grid item sm={6} style={{ textAlign: 'right' }}>
            ...
            <Fab
              ...
              className={classes.button}
              style={selectedDocumentIds.length == 0 ? {} : { color: 'white', background: '#B23535' }}
                    ...
                    >
                      ...
                      <TableCell>
                        <table style={{ padding: 5 }}>
                          <tr>
                            {item.folderId
                              ? <td>{generateIconDocumentFolder(item.shared)}&nbsp;</td>
                              : <td>{generateIconDocument(item.name)}&nbsp;</td>
                            }
                            <td>{item.name}&ensp;{(item.fileId && item.shared) && <MdPeople size={18} />}</td>
                          </tr>
                        </table>
                      </TableCell>
                      <TableCell className={`${classes.cellTableBody}`}>
...
export default compose(
  ...
  withSaga,
  withConnect,
)(withStyles(styles)(MyDriverPage));

...
export function* putMarketUpdate(action) {
  ...
  try {
    const res = yield call(request, urlUpdateMarket, option('PUT', rank));
    // model invalid
...
export function* postMarketAdd(action) {
  ...
  try {
    const res = yield call(request, urlAddMarket, option('POST', rank));
...
export default function* rankData() {
  yield takeLatest(FETCH_RANK_INIT, getMarketInit);
  yield takeLatest(FETCH_RANK_DATA, getMarketData);
  yield takeLatest(UPDATE_RANK, putMarketUpdate);
  yield takeLatest(ADD_RANK, postMarketAdd);
  yield takeLatest(INIT_INDEX_RANK, getInitIndexMarket);
  yield takeLatest(DELETE_RANK_ROW, deleteMarket);
  yield takeLatest(DELETE_MULTIES_RANK, deleteMultiesMarket);
  yield takeLatest(DELETE_MULTIES_RANK_SUCCESS, getMarketData);
  yield takeLatest(DELETE_RANK_SUCCESS, getMarketData);
  yield takeLatest(UPDATE_RANK_SUCCESS, getMarketData);
  yield takeLatest(CREATE_RANK_SUCCESS, getMarketData);
}

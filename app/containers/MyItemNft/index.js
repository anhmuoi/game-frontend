...
export class MarketInformationPage extends React.Component {
  ...
  deleteMultiesMarket = () => {
    const { myNftIdSelected } = this.state;
    this.props.onDeleteMultiesMarket(myNftIdSelected);
  };

  rowsSelected = (ids) => this.setState({ myNftIdSelected: ids });
  ...
  getSortColumn = (sortColumn, value) => {
    const cloneSortColumn = API_COLUMNS.MY_NFTS.indexOf(value);
  ...
  sellItemNft = () => {
    ...
    const nftApi = mapModelMarketUiToApi(this.props.myNftDataModified.toJS());
  ...
  render() {
    ...
    return (
      <div className="room-game">
        ...
        >
          <FormattedMessage {...messages.myNft} />
        </Typography>
        {user ? (
                  ...
                  >
                    ...
                    >
                      ...
                      {item.status === 0 ? (
                        ...
                        <div style={{ textAlign: 'center' }}>
                          <Button
                            onClick={() => this.cancelSellItemNft(item)}
                            style={{
                              background: '#1d1d1d',
                              color: 'white',
                              fontWeight: 'bold',
...
function mapDispatchToProps(dispatch) {
  return {
    ...
    // modified Market
    onUpdateMarket: (id, myNft) => dispatch(putMarketUpdate(id, myNft)),
    onAddMarket: (myNft) => dispatch(postMarketAdd(myNft)),
...
const mapStateToProps = createStructuredSelector({
  ...
  myNftDataModified: getMarketDataModified(),
  userList: getUserListData(),
});

const withReducer = injectReducer({ key: 'myNft', reducer });

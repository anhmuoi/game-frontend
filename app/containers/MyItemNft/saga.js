...
export function* putMarketUpdate(action) {
  ...
  let myNft = action.myNft;
  try {
    const res = yield call(request, urlUpdateMarket, option('PUT', myNft));
    // model invalid
...
export function* postMarketAdd(action) {
  ...
  try {
    const res = yield call(request, urlAddMarket, option('POST', myNft));
 ...
 */
export default function* myNftData() {
  yield takeLatest(FETCH_MY_NFT_INIT, getMarketInit);
  yield takeLatest(FETCH_MY_NFT_DATA, getMarketData);
  yield takeLatest(UPDATE_MY_NFT, putMarketUpdate);
  yield takeLatest(ADD_MY_NFT, postMarketAdd);
  yield takeLatest(INIT_INDEX_MY_NFT, getInitIndexMarket);
  yield takeLatest(DELETE_MY_NFT_ROW, deleteMarket);
  yield takeLatest(DELETE_MULTIES_MY_NFT, deleteMultiesMarket);
  yield takeLatest(DELETE_MULTIES_MY_NFT_SUCCESS, getMarketData);
  yield takeLatest(DELETE_MY_NFT_SUCCESS, getMarketData);
  yield takeLatest(UPDATE_MY_NFT_SUCCESS, getMarketData);
  yield takeLatest(CREATE_MY_NFT_SUCCESS, getMarketData);
}

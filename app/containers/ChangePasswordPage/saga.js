...
export function* putMarketUpdate(action) {
  ...
  let ChangePassword = action.ChangePassword;
  try {
    const res = yield call(
      request,
      urlUpdateMarket,
      option('PUT', ChangePassword),
    );
    // model invalid
...
export function* changePassword(action) {
  const url = `${URL_DOMAIN}/accounts-profile`;
...
export default function* ChangePasswordData() {
  yield takeLatest(FETCH_CHANGE_PASSWORD_INIT, getMarketInit);
  yield takeLatest(FETCH_CHANGE_PASSWORD_DATA, getMarketData);
  yield takeLatest(UPDATE_CHANGE_PASSWORD, putMarketUpdate);
  yield takeLatest(ADD_CHANGE_PASSWORD, postMarketAdd);
  yield takeLatest(REQUEST_JOIN_GROUP, requestJoinGroup);
  yield takeLatest(INIT_INDEX_CHANGE_PASSWORD, getInitIndexMarket);
  yield takeLatest(DELETE_CHANGE_PASSWORD_ROW, deleteMarket);
  yield takeLatest(DELETE_MULTIES_CHANGE_PASSWORD, deleteMultiesMarket);
  yield takeLatest(DELETE_MULTIES_CHANGE_PASSWORD_SUCCESS, getMarketData);
  yield takeLatest(DELETE_CHANGE_PASSWORD_SUCCESS, getMarketData);
  yield takeLatest(UPDATE_CHANGE_PASSWORD_SUCCESS, getMarketData);
  yield takeLatest(CREATE_CHANGE_PASSWORD_SUCCESS, getMarketData);
  ...
  // yield takeLatest(CHANGE_PASSWORD_SUCCESS, getMarketInit);
}
